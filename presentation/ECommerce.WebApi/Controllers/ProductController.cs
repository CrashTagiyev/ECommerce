...
using ECommerce.Application.Behaviors.Query.Product.GetById;
...
public class ProductController : ControllerBase
{
    ...
    {
        GelAllProductsQueryResponse? response = await _mediator.Send(request);
    ...
    public async Task<IActionResult> GetProduct([FromQuery]GetByIdQueryRequest request)
    {
        var responce = await _mediator.Send(request);

        if (responce.ProductVM == null)
            return StatusCode(404);

        return Ok(responce.ProductVM);
    }
}
