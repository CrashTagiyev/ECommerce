...
public class GetProductsQueryHandler : IRequestHandler<GetAllProductsQueryRequest, GelAllProductsQueryResponse>
{
    private readonly IProductService _productService;

    public GetProductsQueryHandler(IProductService productService, ICategoryService categoryService)
    {
        _productService = productService;
    }

    public async Task<GelAllProductsQueryResponse> Handle(GetAllProductsQueryRequest request, CancellationToken cancellationToken)
    {
        var allProductVm = await _productService.GetAllProductsAsync(new PaginationVM() { Page = request.Page, PageSize = request.PageSize });
        return new GelAllProductsQueryResponse(allProductVm);
    }
